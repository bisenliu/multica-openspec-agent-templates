...
- `openspec init` 由用户在使用前手动完成。
- 智能体不要反复检查 init 状态。
- 智能体不要手写 `proposal.md`、specs、`design.md`、`tasks.md` 来替代 OpenSpec 命令。
- Squad 启动后，Leader 和成员通过精确 `@mention` 自动接力。

## 使用前检查

在开始具体需求前，用户只需要手动完成一次：
...
- 当上下文足够时，直接执行当前阶段对应命令，不要回复“请用户运行”来代替执行。
- 如果当前环境不允许执行 Codex OpenSpec 命令，明确标记为 `blocked` 并说明原因。

推荐推进方式：

1. 需求不清楚时，`@需求上下文整理官` 整理上下文，然后等待其回传。
2. 需求足够清楚时，执行 `/prompts:opsx-propose <change-id>`。
3. propose 成功后，`@OpenSpec实施推进官` 自动进入 apply 阶段。
4. apply 成功后，`@OpenSpec归档验收官` 自动进入归档检查。
5. 归档检查通过后，执行 `/prompts:opsx-archive <change-id>`。
...
- 直接执行 `/prompts:opsx-apply <change-id>`；不要只提示用户执行。
...
- 只有在实现完成、测试可信、无阻塞问题时，直接执行 `/prompts:opsx-archive <change-id>`；不要只建议用户运行。
...
- 需求不清楚：`@需求上下文整理官`
- 需求清楚：执行 `/prompts:opsx-propose <change-id>`
- propose 成功：`@OpenSpec实施推进官`
- apply 成功：`@OpenSpec归档验收官`
- 归档检查通过：执行 `/prompts:opsx-archive <change-id>`
...
- 不要把 /prompts:opsx-propose、/prompts:opsx-apply、/prompts:opsx-archive 只作为用户待办输出；可执行时应直接推进。
...
- 需求清楚时，执行 /prompts:opsx-propose <change-id>。
- propose 成功后，@OpenSpec实施推进官。
- OpenSpec实施推进官执行 /prompts:opsx-apply <change-id>，完成后 @OpenSpec归档验收官。
- OpenSpec归档验收官检查通过后，执行 /prompts:opsx-archive <change-id>。
